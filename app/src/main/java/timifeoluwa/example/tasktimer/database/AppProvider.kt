...
const val CONTENT_AUTHORITY = "timifeoluwa.example.tasktimer.provider"
...
//        matcher.addURI(CONTENT_AUTHORITY, "${DurationsContract.TABLE_NAME}/#", TASK_DURATIONS_ID)
    ...
    override fun getType(uri: Uri): String? {
        ...
        return when (uriMatcher.match(uri)) {
            ...
            CURRENT_TIMING -> TimingsContract.CONTENT_ITEM_TYPE
//
...
//            TASK_DURATIONS_ID -> TimingsContract.CONTENT_ITEM_TYPE
    ...
    override fun query(
        ...
        val match =
            uriMatcher.match(uri)  //uriMatcher to work out what kind of URI was given here.uri is the code we will match against. and it returns the code for the matched node
        ...
        when (match) {
            ...
            TASKS_ID -> {
                ...
                val taskId =
                    TasksContract.getId(uri)  //contentProvider doesn't need to know the position of the Id in the path,that's for the contract class to figure out.
...
//            }
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri {
        ...
        if (recordId > 0) {      //this loop notifies the viewModel that data has been inserted and likewise for other functions update and delete.
            //something was inserted
            Log.d(TAG, "insert: Setting notifyChange with $uri")
            context?.contentResolver?.notifyChange(uri, null)
        }//calls the contentResolver to notify the change to the observers (that in the viewModel).the viewModel the observes the contentProvider for a change.
